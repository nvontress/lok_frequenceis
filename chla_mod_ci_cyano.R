...
wgs84 <- 4326 # used for plotting

# Relationship coefficients and constants
coef_tom <- 4050
const_tom <- 20
...
# rename(ntl_validation, MERIS_ci_cyano = `CIcyano Test Set`) %>% 
...
get_wq(station_id = lake_stations, # Requires internet connection
       date_min = '2016-01-01',
       date_max = '2020-12-31',
...
for(i in 2:ncol(ci_extracted_vals)){
  ...
  extract(stack_daily_scaled_filtered[[i-1]],vect(lake_station_coords))[,2] %>% as_tibble() -> ci_extracted_vals[1:nrow(ci_extracted_vals),i]
...
# Join with field values and calculate chl-a
joined_data <- right_join(df,ci_vals) %>% na.omit() %>% 
  mutate(
    chla_tom = coef_tom*ci_val + const_tom,
    chla_seeg = coef_seeg*ci_val + const_seeg
  ) 
...
# Seeger's chla estimates include negative values, which messes with the MAE log calculationss
mae_seeg_daily <- calc_mae_log(joined_data$field_val, joined_data$chla_seeg)
bias_seeg_daily <- calc_bias_log(joined_data$field_val, joined_data$chla_seeg)
