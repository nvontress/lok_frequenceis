...
# load("~/frequency_analysis_ci_cyano_workspace.RData")
...
stack_daily_unscaled_cropped <- rast(stack_daily_unscaled_cropped_w)

stack_weekly_scaled <- rast(stack_weekly_scaled_w)
stack_weekly_unscaled_cropped <- rast(stack_weekly_unscaled_cropped_w)

# Read in land qa flags and apply
land_adjacency_flag <- rast('LandAdjacenyQA.tif') %>% crop(stack_daily_unscaled_cropped) 

stack_daily_unscaled_cropped_masked_2 <-  mask(x = stack_daily_unscaled_cropped[[2]], mask = land_adjacency_flag, maskvalues = 0) # This image contains no data

# Load S3A and S3B flyover dates
load("~/Documents/lok_frequencies/sentinel_records_workspace.RData")

# Set coordinate system
epsg <- crs(stack_daily_unscaled_cropped)
wgs84 <- 'epsg:4326'

lake_bound  <- vect('Shape_WBD/WBDHU12.shp') %>% filter(HUC12 == '030902010200') %>%
  project(epsg)
...
mask(stack_daily_unscaled_cropped,
     land_adjacency_flag,
     maskvalues = 0) -> stack_daily_unscaled_cropped_masked

mask(stack_daily_scaled,
  land_adjacency_flag,
  maskvalues = 0) -> stack_daily_scaled_masked
...
s308c_coords <- read_sf('dbhydro_stations/DBHYDRO_SITE_STATION.shp') %>% clean_names() %>%
  ...
  dplyr::select(station, geometry) %>%
  rename(station_id = station) %>%
  st_transform(crs = epsg) %>% vect()

# Create ROI
roi <- buffer(x = s308c_coords,width = 1502)

# Divide lake into quadrants ----
lake_centroid <- centroids(project(lake_bound,wgs84))
lake_extent <- ext(project(lake_bound,wgs84))

xmn <- lake_extent[1]; xmx <- lake_extent[2]; xmd <- geom(lake_centroid)[1,3]
ymn <- lake_extent[3]; ymx <- lake_extent[4]; ymd <- geom(lake_centroid)[1,4]
...
nw_quad_84 <- st_polygon(x = list(rbind(p1,p2, p5,p4, p1))) %>% 
  st_sfc(crs = wgs84) %>% vect() %>% crop(project(lake_bound,wgs84)) 
...
# Transform quadrants
nw_quad <- project(nw_quad_84,epsg)
ne_quad <- project(ne_quad_84,epsg)
sw_quad <- project(sw_quad_84,epsg)
se_quad <- project(se_quad_84,epsg)
...
# Define breaks
# Original
breaks_daily <- c(min(minmax(stack_daily_scaled_masked),na.rm = T),
                   (((40*0.7)-20)/4050),
                   (((40*1.3)-20)/4050),
                   max(minmax(stack_daily_scaled_masked),na.rm = T))
...
breaks_matrix_daily <- matrix(c(breaks_daily[1], breaks_daily[2], 1,
                                breaks_daily[2], breaks_daily[3], 2,
                                breaks_daily[3], breaks_daily[4], 3),
                              ncol = 3, byrow = T)

stack_daily_classified <- classify(stack_daily_scaled_sub, 
                                   breaks_matrix_daily,
                                   right = NA)
stack_daily_classified_s3a <- classify(stack_daily_scaled_sub_s3a, 
                                       breaks_matrix_daily,
                                       right = NA)
stack_daily_classified_s3a_s3b <- classify(stack_daily_scaled_sub_s3a_s3b, 
                                           breaks_matrix_daily,
                                           right = NA)
...
# Define breaks
# Original
breaks_weekly <- c(min(minmax(stack_weekly_scaled_masked)),
                   (((40*0.7)-20)/4050),
                   (((40*1.3)-20)/4050),
                   max(minmax(stack_weekly_scaled_masked)))
...
## Classify weekly raster images
breaks_matrix_weekly <- matrix(c(breaks_weekly[1], breaks_weekly[2], 1,
                                 breaks_weekly[2], breaks_weekly[3], 2,
                                 breaks_weekly[3], breaks_weekly[4], 3),
                               ncol = 3, byrow = T)

stack_weekly_classified <- classify(stack_weekly_scaled_sub, 
                                    breaks_matrix_weekly,
                                    right = NA)
...
generate_vals_tibble <- function(daily_or_weekly, bound) {
  ...
  exact_extract(stack_classified,st_as_sf(bound)) %>% 
    ...
    dplyr::select(-`NaN`) %>% relocate(date,.before = name) -> tib
  
  exact_extract(stack_unscaled_sub,st_as_sf(bound)) %>%
    first() %>% as_tibble() %>% dplyr::select(-coverage_fraction) %>%
...
# Heat maps for WY 2019 and July - UNUSED CODE----
